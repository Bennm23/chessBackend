...
pub enum AccumulatorRef<'a> {
    Big(&'a mut Accumulator<TRANSFORMED_FEATURE_DIM_BIG>),
    // Small(&'a mut Accumulator<TRANSFORMED_FEATURE_DIM_SMALL>),
}

impl AccumulatorState {
    pub fn get_accumulator_ref<'a>(&'a mut self, dim: usize) -> AccumulatorRef<'a> {
        if dim == TRANSFORMED_FEATURE_DIM_BIG {
            AccumulatorRef::Big(&mut self.big)
        } else {
...
impl AccumulatorState {
    ...
    // pub fn get_big_mut(&mut self) -> &mut Accumulator<TRANSFORMED_FEATURE_DIM_BIG> {
    //     &mut self.big
    // }
    pub fn get_accumulator_mut<const DIM: usize>(&mut self) -> &mut Accumulator<DIM> {
        if DIM == TRANSFORMED_FEATURE_DIM_BIG {
            // (&mut self.big) 
            // &mut self.big as &mut Accumulator<DIM>
            unsafe {
...
// trait AccSelector {
//     type Output;
//     fn get<'a>(&self, state: &'a AccumulatorState) -> &'a Self::Output;
//     fn get_mut<'a>(&self, state: &'a mut AccumulatorState) -> &'a mut Self::Output;
// }

// #[derive(Clone, Copy)]
// struct BigSelector;
// #[derive(Clone, Copy)]
// struct SmallSelector;

// impl AccSelector for BigSelector {
//     type Output = Accumulator<{ TRANSFORMED_FEATURE_DIM_BIG }>;
//     fn get<'a>(&self, state: &'a AccumulatorState) -> &'a Self::Output {
//         &state.big
//     }
//     fn get_mut<'a>(&self, state: &'a mut AccumulatorState) -> &'a mut Self::Output {
...
// impl AccSelector for SmallSelector {
//     type Output = Accumulator<{ SMALL_SIZE }>;
//     fn get<'a>(&self, state: &'a AccumulatorState) -> &'a Self::Output {
//         &state.accumulator_small
//     }
//     fn get_mut<'a>(&self, state: &'a mut AccumulatorState) -> &'a mut Self::Output {
//         &mut state.accumulator_small
//     }
// }

// fn with_acc<S, F, R>(state: &mut AccumulatorState, selector: S, f: F) -> R
// where
//     S: AccSelector,
//     F: FnOnce(&mut S::Output) -> R,
// {
//     let acc = selector.get_mut(state);
//     f(acc)
// }
...
fn update_accumulator_refresh_cache<const DIM: usize>(
    ...
    board: &Board,
    // accum_getter: AccumSelector<DIM>,
    // accum_state: &mut AccumulatorState,
    ...
    // let mut removed_features: half_ka_v2_hm::IndexList = [0; half_ka_v2_hm::MAX_ACTIVE_DIMENSIONS];
    // let mut added_features: half_ka_v2_hm::IndexList = [0; half_ka_v2_hm::MAX_ACTIVE_DIMENSIONS];
    ...
    //     println!("  Removing feature index {}", index);
    // }
    // for index in &added_features {
    //     println!("  Adding feature index {}", index);
    // }
...
impl AccumulatorStack {
    ...
    fn forward_update_incremental<const DIM: usize>(
        &mut self, perspective: Player, board: &Board, ft: &FeatureTransformer<DIM>, start_index: usize, 
        // selector: impl AccSelector
    ) {
        ...
        // with_acc(&mut self.accumulators[start_index], selector, |acc| {
        //     assert!(acc.computed[perspective as usize]);
        // });
    ...
    fn backward_update_incremental<const DIM: usize>(
        &mut self, perspective: Player, board: &Board, ft: &FeatureTransformer<DIM>, end_index: usize, 
        // selector: impl AccSelector
    ) {
        ...
        // for next in (end_index + 1)..self.current_index {
           update_accumulator_incremental::<DIM>(
                perspective,
                Direction::Backward,
                ft,
...
pub struct CacheEntry<const DIM: usize> {
    ...
    pub by_color_bb: [BitBoard; COLORS],      // bitboards if you need them
    pub by_type_bb: [BitBoard; PIECE_TYPE_NB],            // adjust size to your piece types
}
